...
// global
let i = {iterations: 1, size: 2, angle: Math.PI/6};
...
const materials = [new THREE.MeshPhongMaterial({color: 0x202025}),
    new THREE.MeshPhongMaterial({color: 0x402015})];
const newSprite = () => {
...
function drawTree(n, size, root) {
    ...
    drawTree(n - 1, size/1.25, drawBranch(size, new THREE.Vector3(i.angle, i.angle, i.angle), root));
    drawTree(n - 1, size/1.25, drawBranch(size, new THREE.Vector3(-supp(i.angle), -supp(i.angle), -supp(i.angle)), root));
